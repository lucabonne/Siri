...
- `jarvis voice logs` inspects recent local structured voice events from the configured JSONL file, with optional event/status/outcome filters, approval/dispatch-only filtering, `--limit`, and `--json`. It does not record, call the API, dispatch, approve, speak, or start hotkeys. Voice logs do not store raw audio and summarize transcripts by length/hash plus a redacted preview unless full transcript logging is explicitly enabled.

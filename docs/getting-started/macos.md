...
- Mission Control reads the same recorder diagnostics from `/v1/voice/ptt/status` and separately shows `dev-silent` availability, real microphone configuration, `sounddevice` dependency availability, preview-only microphone pipeline availability, and approval-gated microphone dispatch availability. Readiness requires a statically available real microphone backend; `dev-silent` alone is not reported as microphone-ready. Mission Control does not open or probe the microphone, determine permission status, record audio, mutate settings, or execute commands.

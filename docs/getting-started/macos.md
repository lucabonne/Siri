...
- `jarvis voice logs` shows recent local structured voice events from the configured JSONL file. Voice logs do not store raw audio and summarize transcripts by length/hash plus a redacted preview unless full transcript logging is explicitly enabled.

...
- `jarvis voice hotkey-bridge` only prints a bounded, preview-only-by-default `jarvis voice mic-run` command or a disabled Hammerspoon F18 example that an external helper could call after manual setup. It omits `--approve-dispatch` and `--speak-result`; it does not install the example, start a global listener, capture Fn/F18, request **Accessibility** permission, record audio, dispatch, approve, or speak. Approved-dispatch and speech variants remain separate manual `mic-run` terminal opt-ins and are not generated into the bridge.

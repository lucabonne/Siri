...
- `jarvis voice logs` inspects recent local structured voice events from the configured JSONL file, with optional event/status/outcome filters, approval/dispatch-only filtering, `--limit`, `--json`, and explicit `--export PATH` support for JSONL or JSON local files. It does not record, call the API, dispatch, approve, speak, or start hotkeys. Voice logs and exports do not store raw audio and summarize transcripts by length/hash plus a redacted preview unless full transcript logging was explicitly enabled before the event was written. Export parent directories must already exist.

...
- Future Hammerspoon or Swift Fn/push-to-talk helpers will require macOS **Accessibility** permission for global hotkey capture. OpenJarvis does not request it programmatically. Any manually enabled helper that invokes `jarvis voice hotkey-runtime --trigger --recorder macos` or `jarvis voice mic-run --recorder macos` will also require **Microphone** permission.

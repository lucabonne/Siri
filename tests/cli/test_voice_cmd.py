...
def test_voice_release_readiness_help_text_stays_manual_and_safe() -> None:
    ...
    assert "--manual-test" in bridge_output
...
def test_voice_hotkey_runtime_requires_contract_flag() -> None:
    ...
    assert (
        "pass exactly one of --contract, --dry-run, --preflight, --status, or --trigger"
        in result.output
    )
...
def test_voice_hotkey_runtime_status_reports_ready_configuration(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        voice_cmd,
    ...
    result = CliRunner().invoke(voice_cmd.voice, ["hotkey-runtime", "--status"])

    assert result.exit_code == 0
    assert "Voice hotkey runtime status" in result.output
    assert "ready: True" in result.output
    assert (
    ...
    assert "--base-url http://127.0.0.1:9000" in result.output
    assert "--session-id fn-preview" in result.output
    assert "runtime contract available: True" in result.output
    assert "dry-run available: True" in result.output
    assert "preflight available: True" in result.output
    assert "trigger command available: True" in result.output
    assert "preview-only default: True" in result.output
    assert "dispatch requires explicit approval: True" in result.output
    assert (
        "speech requires explicit dispatch approval plus speech opt-in: True"
        in result.output
    )
    assert "recorder readiness: True (macos)" in result.output
    assert "duration readiness: True (1.5 seconds)" in result.output
    assert "transcription/model readiness: True" in result.output
    assert "API base URL readiness: True (http://127.0.0.1:9000)" in result.output
    assert "event logging state: enabled=True" in result.output
    assert "status command writes event=False" in result.output
    assert "listener started by Python: False" in result.output
    assert "init.lua mutated by Jarvis: False" in result.output
    assert "Accessibility permission requested by Jarvis: False" in result.output
    assert "--approve-dispatch" not in result.output
    assert "--speak-result" not in result.output


def test_voice_hotkey_runtime_status_reports_missing_recorder_and_transcription(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        voice_cmd,
    ...
    assert result.exit_code == 0
    assert "ready: False" in result.output
    assert "recorder readiness: False (dev-silent)" in result.output
    assert "transcription/model readiness: False" in result.output
    assert "[voice_control].hotkey_bridge_recorder must be macos or sounddevice" in (
        result.output
    )
    assert "local transcription adapter is not configured" in result.output
...
def test_voice_hotkey_runtime_status_json_reports_audit_and_safety(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        voice_cmd,
    ...
    assert data["version"] == 1
    assert data["ready"] is True
    assert data["runtime_capabilities"] == {
        "contract_available": True,
        "dry_run_available": True,
        "preflight_available": True,
        "trigger_command_available": True,
    }
    assert data["generated_hammerspoon_bridge_expected_argv"][:4] == [
        "jarvis",
        "voice",
        "hotkey-runtime",
        "--trigger",
    ]
    assert data["generated_hammerspoon_bridge_expected_command"].startswith(
        "jarvis voice hotkey-runtime --trigger"
    )
    assert data["internal_preview_argv"][:3] == ["jarvis", "voice", "mic-run"]
    assert (
        "--approve-dispatch" not in data["generated_hammerspoon_bridge_expected_argv"]
    )
    assert "--speak-result" not in data["generated_hammerspoon_bridge_expected_argv"]
    assert data["checks"]["runtime_contract_available"]["passed"] is True
    assert data["checks"]["dry_run_available"]["passed"] is True
    assert data["checks"]["preflight_available"]["passed"] is True
    assert data["checks"]["trigger_command_available"]["passed"] is True
    assert data["checks"]["preview_only_default"]["passed"] is True
    assert data["checks"]["dispatch_requires_explicit_approval"]["passed"] is True
    assert (
        data["checks"]["speech_requires_dispatch_approval_plus_speech_opt_in"]["passed"]
        is True
    )
    assert data["checks"]["recorder_readiness"]["passed"] is True
    assert data["checks"]["transcription_model_readiness"]["passed"] is True
    assert data["checks"]["api_base_url_readiness"]["passed"] is True
    assert data["checks"]["event_logging_state"]["status_command_writes_event"] is False


def test_voice_hotkey_runtime_status_has_no_side_effect_calls(
    monkeypatch,
) -> None:
    def fail_side_effect(*args, **kwargs):
        raise AssertionError("hotkey runtime status must not run voice flow")
    ...
    assert result.exit_code == 0
    assert "Safety: no recording, transcription, submit" in result.output


def test_voice_hotkey_runtime_status_json_reports_false_safety_fields(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        voice_cmd,
    ...
    assert result.exit_code == 0
    safety = json.loads(result.output)["safety"]
    assert safety["listener_started_by_python"] is False
    assert safety["init_lua_mutated_by_jarvis"] is False
    assert safety["accessibility_permission_requested_by_jarvis"] is False
    assert safety["record_audio"] is False
    assert safety["transcribe"] is False
    assert safety["submit"] is False
    assert safety["dispatch"] is False
    assert safety["speak"] is False
    assert safety["lua_executed"] is False
    assert safety["bridge_shell_commands_run"] is False
    assert safety["event_logged"] is False
...
def test_voice_hotkey_bridge_activation_guide_prints_preflight_and_rollback(
    ...
    monkeypatch.setattr(
        ...
        "_log_voice_event",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("activation guide must not write voice logs")
        ),
    )
    ...
    assert "Manual install steps:" in result.output
    assert dofile_line in result.output
    assert "Manual rollback steps:" in result.output
    assert f"Remove this exact line from {active_init}" in result.output
    assert (
...
def test_voice_hotkey_bridge_activation_guide_validation_failure_has_no_steps(
    tmp_path,
) -> None:
    bridge_path = tmp_path / "unsafe-dispatch.lua"
    _write_hammerspoon_validation_fixture(
...
def test_voice_hotkey_bridge_activation_guide_does_not_mutate_files(
    monkeypatch, tmp_path
) -> None:
    home = tmp_path / "home"
    active_init = home / ".hammerspoon" / "init.lua"
    ...
    def fail_side_effect(*args, **kwargs):
        raise AssertionError("activation guide must not start voice flow")
...
def test_voice_hotkey_bridge_activation_guide_rejects_format_option(tmp_path) -> None:
    bridge_path = tmp_path / "openjarvis-voice.lua"
    _write_hammerspoon_validation_fixture(bridge_path)
...
def test_voice_hotkey_bridge_manual_test_prints_step_by_step_flow(
    monkeypatch, tmp_path
) -> None:
    bridge_path = tmp_path / "openjarvis-voice.lua"
    _write_hammerspoon_validation_fixture(bridge_path)
    ...
    monkeypatch.setattr(
        ...
        "_log_voice_event",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("manual test must not write voice logs")
        ),
    )

    result = CliRunner().invoke(
        voice_cmd.voice,
        ["hotkey-bridge", "--manual-test", str(bridge_path)],
    )

    assert result.exit_code == 0
    assert (
        "Hammerspoon activation manual test (user-performed outside Jarvis)"
        in result.output
    )
    assert "scope: printed guide only; Jarvis does not run these steps" in (
        result.output
    )
    assert "1. Run safe diagnostics:" in result.output
    assert "jarvis voice doctor" in result.output
    assert "2. Run the runtime readiness preflight:" in result.output
    assert "jarvis voice hotkey-runtime --preflight" in result.output
    assert "3. Generate or validate the Hammerspoon bridge manually:" in (result.output)
    assert "jarvis voice hotkey-bridge --write-hammerspoon" in result.output
    assert "jarvis voice hotkey-bridge --validate-hammerspoon" in result.output
    assert "4. Manually add this exact line to ~/.hammerspoon/init.lua:" in (
        result.output
    )
    assert "5. Manually reload Hammerspoon from the menu bar:" in result.output
    assert "6. Press the configured Hammerspoon hotkey." in result.output
    assert "8. Inspect local voice events:" in result.output
    assert "jarvis voice logs" in result.output


def test_voice_hotkey_bridge_manual_test_reports_preview_only_default(
    tmp_path,
) -> None:
    bridge_path = tmp_path / "openjarvis-voice.lua"
    _write_hammerspoon_validation_fixture(bridge_path)
...
def test_voice_hotkey_bridge_manual_test_prints_rollback_instructions(
    tmp_path,
) -> None:
    bridge_path = tmp_path / "openjarvis-voice.lua"
    _write_hammerspoon_validation_fixture(bridge_path)
    ...
    assert result.exit_code == 0
    assert "9. Roll back manually if needed:" in result.output
    assert "jarvis voice hotkey-bridge --rollback-guide" in result.output
    assert "Remove this exact line manually from" in result.output
    assert dofile_line in result.output
    assert "local enable_openjarvis_voice_hotkey = false" in result.output
    assert "reload Hammerspoon manually" in result.output


def test_voice_hotkey_bridge_manual_test_does_not_mutate_files(
    monkeypatch, tmp_path
) -> None:
    home = tmp_path / "home"
    active_init = home / ".hammerspoon" / "init.lua"
    ...
    def fail_side_effect(*args, **kwargs):
        raise AssertionError("manual test must not start voice flow")
...
def test_voice_hotkey_bridge_manual_test_prints_safety_statements(
    tmp_path,
) -> None:
    bridge_path = tmp_path / "openjarvis-voice.lua"
    _write_hammerspoon_validation_fixture(bridge_path)
    ...
    assert result.exit_code == 0
    assert "Safety boundaries for this manual-test command:" in result.output
    assert "Lua execution by Jarvis: not attempted" in result.output
    assert "shell commands from bridge by Jarvis: not run" in result.output
    assert "listener started by Jarvis: False" in result.output
...
def test_voice_hotkey_bridge_activation_status_reports_ready(
    monkeypatch, tmp_path
) -> None:
    home = tmp_path / "home"
    home.mkdir()
    ...
    monkeypatch.setattr(
        ...
        "_log_voice_event",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("activation status must not write voice logs")
        ),
    )
...
def test_voice_hotkey_bridge_activation_status_reports_missing_bridge(
    ...
    assert result.exit_code == 0
    assert "generated bridge exists: False" in result.output
...
def test_voice_hotkey_bridge_activation_status_prints_manual_only_safety(
    monkeypatch, tmp_path
) -> None:
    home = tmp_path / "home"
    active_init = home / ".hammerspoon" / "init.lua"
    ...
    def fail_side_effect(*args, **kwargs):
        raise AssertionError("activation status must not start voice flow")
    ...
    assert active_init.read_text(encoding="utf-8") == original_init
    assert (
        "Accessibility permission: user-managed; not requested by Jarvis"
        in result.output
    )
    assert (
